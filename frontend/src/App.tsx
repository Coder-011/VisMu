      ...
      return (
        <div className="flex items-center justify-center min-h-screen w-screen bg-[#050505] text-white p-4">
          <div className="text-center space-y-4 p-8 border border-red-900/50 rounded-2xl bg-[#0a0a0a] max-w-md w-full">
            ...
            <button onClick={() => window.location.reload()} className="px-6 py-2 bg-[#00f2ff] text-black rounded-lg font-bold text-sm">
              Reload
            </button>
          </div>
        </div>
...
const App: React.FC = () => {
  ...
  const handleSidebarItemClick = (label: string) => {
    ...
    setShowSidebarMessage(`${label} coming soon...`);
        ...
        {!initialized && (
          <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center backdrop-blur-md p-4">
            <div className="text-center space-y-8 w-full max-w-md p-8 sm:p-12 border border-[#1a1a1a] rounded-3xl bg-[#0a0a0a] shadow-2xl">
              <h1 className="text-5xl sm:text-6xl font-black italic tracking-tighter text-[#00f2ff]">VisMu</h1>
        ...
        <div className="flex-1 flex flex-col min-w-0 w-full">
          <header className="h-14 sm:h-16 border-b border-[#1a1a1a] flex items-center justify-between px-4 sm:px-8 bg-[#0a0a0a] shrink-0 relative">
            <div className="flex items-center space-x-4 sm:space-x-12">
              <button className="lg:hidden text-gray-400 hover:text-white mr-2" onClick={() => setSidebarOpen(true)}>
                <Menu size={20} />
              </button>
              <h1 className="text-xl sm:text-2xl font-black italic tracking-tighter text-[#00f2ff]">VisMu</h1>
                ...
                {['Studio', 'Library', 'Live', 'Nodes'].map((item) => (
                  <button
                    ...
                    className={`text-[11px] font-bold tracking-widest uppercase pb-1 transition-all ${
                      item === activeTab ? 'text-white border-b-2 border-[#00f2ff]' : 'text-gray-500 hover:text-gray-300'
                    }`}
                  >
                    {item}
                  </button>
            ...
            <div className="flex items-center space-x-4 sm:space-x-6 text-gray-500">
              ...
              <button
                className="hover:text-[#00f2ff] transition-colors relative"
                onClick={() => { setShowSettings(!showSettings); setShowUserMenu(false); }}
              >
                <Settings size={18} />
                {showSettings && (
                  <div className="absolute top-10 right-0 w-56 bg-[#0a0a0a] border border-[#1a1a1a] rounded-xl p-4 shadow-2xl z-50">
                    <h3 className="text-xs font-bold text-white mb-3">Settings</h3>
                    <div className="space-y-3 text-[10px] text-gray-400">
                      <p>Hand tracking: <span className="text-[#00f2ff]">Client-side (MediaPipe)</span></p>
                      <p>Audio: <span className="text-[#00f2ff]">Web Audio API</span></p>
                      <p>Storage: <span className="text-[#00f2ff]">localStorage</span></p>
                      <button
              ...
              >
                <User size={18} />
                {showUserMenu && (
                  <div className="absolute top-10 right-0 w-44 bg-[#0a0a0a] border border-[#1a1a1a] rounded-xl p-4 shadow-2xl z-50">
          ...
          <main className="flex-1 overflow-auto relative">
            ...
            {activeTab === 'Library' && <div className="p-8 text-center text-gray-500 text-sm">Library coming soon...</div>}
            {activeTab === 'Live' && <div className="p-8 text-center text-gray-500 text-sm">Live mode coming soon...</div>}
            {activeTab === 'Nodes' && <div className="p-8 text-center text-gray-500 text-sm">Node editor coming soon...</div>}
            {showSidebarMessage && (
              <div className="fixed bottom-20 left-1/2 -translate-x-1/2 bg-[#00f2ff] text-black px-4 py-2 rounded-lg text-xs font-bold z-50 shadow-lg">
                {showSidebarMessage}
              </div>
            )}
          </main>
